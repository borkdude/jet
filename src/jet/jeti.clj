...
(defn print-help [sub-command]
  (if sub-command
    (case sub-command
      :jeti/slurp
      (println ":jeti/slurp <file> [{:format ..., :keywordize ...}]: slurps a file into the shell from disk. :format is one of :json, :edn ,transit, :csv or :tsv (defaults to :edn) and :keywordize is a boolean that only applies to :json.")
      :jeti/spit
      (println ":jeti/spit <file> [{:format ..., :pretty ...}]: spits a file to disk. :format is one of :json, :edn, :transit, :csv or :tsv (defaults to :edn) and :pretty is a boolean that indicates if the output should be pretty printed.")
...
(defn read-file [state current-id file {:keys [:format :keywordize]
  ...
  (try (let [next-id (new-id state)
             ...
             file-as-edn (case format
                           ...
                           :csv (csv/read-csv file-as-string)
                           :tsv (csv/read-csv file-as-string :separator \tab))]
...
(defn write-file [value file {:keys [:format :pretty]
  ...
  (try (let [out-string (case format
                          ...
                          :csv (doto (StringWriter.) (csv/write-csv value) str)
                          :tsv (doto (StringWriter.) (csv/write-csv value :separator \tab) str))]
